...
maximal_cliques=BronKerbosch(G)
...
for start_id, end_id, data in G1.edges(data=True):
    ...
    cursor.execute(sql, (start_id, end_id, weight, 2020))

plt.figure(figsize=(8,6))
pos = nx.spring_layout(G1)
nx.draw(G1,pos,with_labels=True,node_color='lightblue',node_size=2000,font_size=10,font_color='black',edge_color='gray')
plt.title("visualization of Graph G1")
plt.show()

maximal_cliques=BronKerbosch(G1)
...
for i,clique in enumerate(maximal_cliques):
    ...
    clique_to_node1[frozenset(clique)]=new_node

for clique1,node1 in clique_to_node1.items():
    for clique2,node2 in clique_to_node1.items():
        if node1 != node2:
            connected=False
            for node_in_clique1 in clique1:
                for node_in_clique2 in clique2:
                    if G1.has_edge(node_in_clique1,node_in_clique2):
                        connected=True
                        break
                if connected:
                    break
...
nx.draw(G2,pos,with_labels=True,node_color='lightblue',node_size=2000,font_size=10,font_color='black',edge_color='gray')
plt.title("visualization of Graph G2")
plt.show()

maximal_cliques=BronKerbosch(G2)
...
for i,clique in enumerate(maximal_cliques):
    ...
    clique_to_node2[frozenset(clique)]=new_node

for clique1,node1 in clique_to_node2.items():
    for clique2,node2 in clique_to_node2.items():
        if node1 != node2:
            connected=False
            for node_in_clique1 in clique1:
                for node_in_clique2 in clique2:
                    if G2.has_edge(node_in_clique1,node_in_clique2):
                        connected=True
                        break
                if connected:
                    break
...
nx.draw(G3,pos,with_labels=True,node_color='lightblue',node_size=2000,font_size=10,font_color='black',edge_color='gray')
plt.title("visualization of Graph G2")
plt.show()
